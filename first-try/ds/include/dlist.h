...
/* return the return-value of the operation applied to the last iter (iter_end or DListEnd)
, complexity O(n) */
int DListForEach(dlist_iter_t iter_start, dlist_iter_t iter_end,
                    oper_func_t operation, void* param); 

/*return the first found iter or iter_end if not found, complexity O(n)*/
dlist_iter_t DListFind(dlist_iter_t iter_start, dlist_iter_t iter_end,
                       is_match_func match, const void* param);
...
/*insert before iter, return the inserted iter, complexity O(1)*/
